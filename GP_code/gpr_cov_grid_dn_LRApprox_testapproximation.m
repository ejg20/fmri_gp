...
function [nlml, dnlml, alpha_kron, Qs, V_kron] = gpr_cov_grid_dn_LRApprox_testapproximation(logtheta, xgrid, input, noise_var, cov)


for test = 1:20
    
    logtheta = [randn(2,1)*2+1;randn*2+4];
    %%%
    % function parameters
    %
    ...
    if( sphericalNoise )
        ...
        V_cumsum = (cumsum(V_kron_sort)/sum(V_kron_sort));
        m = find(V_cumsum>0.999,1);
        V_approx = (R)*V_kron_sort(1:m);
        logdet_kron =  real(sum(log(V_approx+noise_var_approx)));
        %      return
    ...
    results =  [temp_findlogdetapprox(V_kron_sort,Ks,input,m,noise_var_approx,V_kron,D,Qs,N,n)',m]
    approx_results(test,:) = results;
    ...
    for ell_outloop = 1:D
        ...
        %     toc
        dnlml(ell_outloop) = 0.5*(R*sum(diag_Z(V_index_to_N(1:m))./(V_approx+noise_var_approx)) - alpha_kron_to_N'*kron_mv(dK, alpha_kron_to_N));
        %
    ...
    % toc
    dnlml(D+1) = 0.5*(R*sum(diag_Z(V_index_to_N(1:m))./(V_approx+noise_var_approx)) - alpha_kron_to_N'*kron_mv(dK, alpha_kron_to_N));
    ...
    if(learnNoiseFlag)
        ...
        dnlml(D+2) = 0.5*(sum(1./(V_approx+noise_var_approx)) - alpha_kron'*alpha_kron);
    ...
    dnlml = real(dnlml);
    %  disp(logtheta');
    if(size(dnlml,1)>size(dnlml,2))
        disp_dnlml = dnlml';
    else
        disp_dnlml = dnlml;
    end
    disp([nlml,datafit,complex_penalty,disp_dnlml]);
    disp([logtheta']);
