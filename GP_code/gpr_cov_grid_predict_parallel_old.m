...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [mu_f, var_f] = gpr_cov_grid_predict_parallel(xstars, logtheta, xgrid, input, alpha, noise, cov, Qs, V_kron)
...
for d=1:D
    ...
    Kxy_sparse{d,1} = feval(cov{:},hyp_cov, xg, b);
    % save the reference to all xstars with this input in this dimension
    Kxy_sparse{d,2} = n;
    if(std_flag)
        Kxtxt = Kxtxt*(feval(cov{:},hyp_cov, 1));
    end
end
...
if(std_flag)
    ...
    parfor m = 1:M
        ...
        if(std_flag)
            ...
            % fast calculation of alpha_kron = (Kxx+sn^2)^-1*Kxx* using PCG
            %[alpha_kron rs] = pre_conj_grad_solve(Qs, V_kron, noise, Kstar_kron_input, C, max_iter);
            [alpha_kron rs] = pre_conj_grad_solve(Qs, V_kron, noise, Kstar_kron(index_to_N),index_to_N, C, max_iter,1e-3);
            alpha_to_N = zeros(N,1);
            alpha_to_N(index_to_N)=alpha_kron;
            % calculate posterior variance = Kx*x* - Kx*x (Kxx-sn^2)^-1 Kxx*
            var_f(m) = Kxtxt - Kstar_kron'*alpha_to_N;
