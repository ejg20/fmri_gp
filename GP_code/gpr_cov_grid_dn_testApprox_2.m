...
function [datafit,complex_penalty,datafit_true,complex_penalty_true,tr_true,tr_approx_sf,m] = gpr_cov_grid_dn_testApprox_2(logtheta, xgrid, input, noise_var, cov,R,dummyNoise)
datafit=0;
complex_penalty=0;
datafit_true=0;
...
figure(2);subplot(1,2,1);plot(V_kron_sort);
ylim1 =  get(gca,'ylim');
...
%         noise_var_approx = real(exp(sum(log(noise_var(input.index_to_N)))/n));
    ...
    complex_penalty_true = (logdet_kron_true);
