function [hypers_learned nlml] = gp_grid_cv(nfolds, input, gpmodel, vset)
%
...
%         [tr, tst, xstar] = gp_grid_set_splitsets(cvsets{cvtestnum}, input, gpmodel);
        ...
        tr.gpmodel = gp_grid_gpmodel_class(gpmodel);
        tst.gpmodel = gp_grid_gpmodel_class(gpmodel);
         
        tr.gpmodel.logpriorfuns{1}.func = @(t) gp_grid_Laplace(t, lambda);
        tr.gpmodel.logpriorfuns{1}.indices = 1:floor(gp_grid_numofhyps(gpmodel.hyps_in_d)/3);
        
        tst.gpmodel.logpriorfuns{1}.func = @(t) gp_grid_Laplace(t, lambda);
        tst.gpmodel.logpriorfuns{1}.indices = 1:floor(gp_grid_numofhyps(gpmodel.hyps_in_d)/3);
        
        tst.gpmodel.noise_struct.sphericalNoise = tr.input.isgridcomplete();
        tr.gpmodel.noise_struct.sphericalNoise = tst.input.isgridcomplete();
        ...
        params.wm = std(tr.input.zeromeandata);
        params.sm = 2*Gs./input.Fs;
        tr.gpmodel.hyperparams = zeros(numberstarts,sum(gp_grid_numofhyps(gpmodel.hyps_in_d))+1);
        tr.gpmodel.learn = true;
        for numoftries = 1:4    % give a few tries just if initial values are very bad and can't converge
            for nrandstart=1:numberstarts
                tr.gpmodel.hyperparams(nrandstart,:) = [make_gpsmp_hyps_initvals(input, gpmodel,3,params)', -1];
            end
            [hypers_learned, trnlml, Iout] = gpgrid(tr.input, tr.gpmodel, xstar,maxiteration);
            
            if(trnlml.numofitr>maxiteration*0.02)    % valid optimization if num of iterations excided 5% of allowed iterations
                break;
            end
        end
