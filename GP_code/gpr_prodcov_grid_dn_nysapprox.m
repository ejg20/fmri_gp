%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% gpr_cov_grid_dn_nysapprox()
%
% Fast product covariance matrix calculation for GP-grid using Nystrom
% approximation
%
% Usage: [nlml, dnlml] = gpr_prodcov_grid_dn_nysapprox(hypvec, xgrid, input, noise_struct, cov, hyps_in_dim)
%        [nlml, dnlml, alpha_kron, Qs, V_kron] = gpr_prodcov_grid_dn_nysapprox(hypvec, xgrid, input, noise_struct, cov, hyps_in_dim)
%
% Inputs:
...
function [nlml, dnlml, alpha_kron, Qs, V_kron] = gpr_prodcov_grid_dn_nysapprox(hypvec, xgrid, input, noise_struct, cov, hyps_in_dim)
...
% locations.
if( isstruct(noise_struct) )
    if(noise_struct.learn)
        learnNoiseFlag = true;
        %check if the noise matrix is an eye matrix (spherical noise)
        if(noise_struct.sphericalNoise)
            sphericalNoise = true;
        end
    end
...
for d = 1:D
    ...
    hlpvar=[];
    if(nargout(cov{1}) == 1)
        [K] = feval(cov{:},hyp_val, xg); %TODO: Toeplitz
    elseif(nargout(cov{1}) == 2)
        [K, hlpvar] = feval(cov{:},hyp_val, xg); %TODO: Toeplitz
    end
...
if( sphericalNoise )
    ...
    [alpha_kron rs] = pre_conj_grad_solve(Qs, V_kron, noise_struct.var,...
...
% lambda=noise_struct.lambda;
% num_of_w = ((length(hypvec)-1)/3);
% nlml = nlml + lambda*sum(exp(hypvec(1:num_of_w)))-length(hypvec)*log(lambda/2); %sign in order to not use when 0
% 
...
for d = 1:D
    ...
    for hypd_i = 1:length(hyp_val)
        ...
        if(isempty(hlpvar))
            ...
            [dK{d}]  = feval(cov{:},hyp_val, xg,[],hypd_i, hlpvar);
        end
...
%     hyps_in_d_mat = sortrows(hyps_in_dim{d},1);
...
% fftxgrid{1} = -max(xgrid{1}):0.1:max(xgrid{1});
% fftxgrid{2} = -max(xgrid{2}):0.1:max(xgrid{2});



fftxgrid{1} = -.50:0.01:.50;
fftxgrid{2} = -.50:0.01:.50;
% fftxgrid = xgrid;

% specdensityImage = multdimSpecdensity(fftxgrid,hypvec(:),hyps_in_dim);
